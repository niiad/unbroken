...
@Component
public class WithdrawFromCashAccountController implements Initializable {
    ...
    public WithdrawFromCashAccountController(TransactionUseCase transactionUseCase) {
        this.transactionUseCase = transactionUseCase;
    }

    @FXML
    void makeWithdrawal(MouseEvent mouseEvent) {
        if (mouseEvent.getSource() == addButton) {
            ...
            if (selectedCashAccountDisplay != null) {
                ...
                if (actualCashAccount != null) {
                    if (Double.valueOf(amountField.getText()) > actualCashAccount.getBalance()) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);

                        alert.setContentText("Not enough balance to complete withdrawal!");
                    ...
                    Transaction transaction = new Transaction(0L, actualCashAccount.getAccountName(),
                            receiverField.getText(), Double.valueOf(amountField.getText()), "withdraw", timeOfTransaction.format(dateFormatter), "");

                    boolean status = cashAccountOutputPort.withdrawFromCashAccount(actualCashAccount, Double.valueOf(amountField.getText()));

                    Transaction performedTransaction = transactionUseCase.performTransaction(transaction);

                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION);

                    alert.setContentText("New withdrawal made " + status + ", transaction performed under type " + performedTransaction.getType());
                    alert.setTitle("Status");
    ...
    @FXML
    void cancelMakeWithdrawal(MouseEvent mouseEvent) {
